function varargout = GUI_MT218(varargin)
%The graphical user interface for the DP system, start here

gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @GUI_MT218_OpeningFcn, ...
    'gui_OutputFcn',  @GUI_MT218_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before GUI_MT218 is made visible.
function GUI_MT218_OpeningFcn(hObject, eventdata, handles, varargin)
warning off
assignin('base','TS_wii',str2double(get(handles.edt_wiisample,'String')))
assignin('base','TS_boat',str2double(get(handles.edt_fugisample,'String')))
...
function varargout = GUI_MT218_OutputFcn(hObject, eventdata, handles)
...
calibration = calibratewiimotes(wiimotes);
...
outtoGUI(['selected port: ',ports.names{get(hObject,'Value')}])
...
fprintf(s,'D')
pause(.5)
voltage = str2num(fscanf(s))/102.3;
